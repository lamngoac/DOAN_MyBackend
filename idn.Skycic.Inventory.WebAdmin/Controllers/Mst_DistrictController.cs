...
{
    public class Mst_DistrictController : BaseController
    {
        // GET: Mst_District
        [HttpGet]
        public ActionResult Index(string init = "init", int page = 0)
        {
            ...
            if(true)
            {
                ...
                if (init != "search")
                {
                    ...
                    var objRT_Mst_DistrictCur = Mst_DistrictService.Instance.WA_RptSv_Mst_District_Get(objRQ_Mst_District, addressAPIs);
        ...
        public ActionResult Create(string model)
        {
            ...
            try
            {
                ...
                if (true)
                {
                    ...
                    Mst_DistrictService.Instance.WA_RptSv_Mst_District_Create(objRQ_Mst_District, addressAPIs);
                ...
                if (true)
                {
                    if (!CUtils.IsNullOrEmpty(model))
                    {
                        ...
                        if (objRT_Mst_DistrictCur.Lst_Mst_District != null && objRT_Mst_DistrictCur.Lst_Mst_District.Count > 0)
                        {
                            ...
                            Mst_DistrictService.Instance.WA_RptSv_Mst_District_Update(objRQ_Mst_District, addressAPIs);

                            resultEntry.Success = true;
                            resultEntry.AddMessage("Sửa Quận huyện thành công!");
                            resultEntry.RedirectUrl = Url.Action("Index");
                        }
                        else
                        {
                            resultEntry.Success = true;
                            resultEntry.AddMessage("Mã quận huyện '" + objMst_DistrictInput.DistrictCode + "' - '" + objMst_DistrictInput.ProvinceCode + "' không có trong hệ thống!");
                        }
                    }
                    else
                    {
                        resultEntry.Success = true;
                        resultEntry.AddMessage("Mã quận huyện trống!");
                    }
                    return Json(resultEntry);
                }
                else
            ...
            if (true)
            {
                if (!CUtils.IsNullOrEmpty(districtcode))
                {
                    ...
                    if (objRT_Mst_DistrictCur.Lst_Mst_District != null && objRT_Mst_DistrictCur.Lst_Mst_District.Count > 0)
                    {
                        ...
                        Mst_DistrictService.Instance.WA_RptSv_Mst_District_Delete(objRQ_Mst_District, addressAPIs);
                    ...
                    {
                        resultEntry.Success = true;
                        resultEntry.AddMessage("Mã quận huyện '" + districtcode + "' không có trong hệ thống!");
                    }
                }
                else
                {
                    ...
                    if (table != null && table.Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (DataRow dr in table.Rows)
                            {
                                ...
                                {
                                    exitsData = "Tên quận huyện không được trống!" + strRows;
                                    resultEntry.AddMessage(exitsData);
                                    return Json(resultEntry);
                                }
                            ...
                            for (var i = 0; i < table.Rows.Count; i++)
                            {
                                ...
                                for (var j = 0; j < table.Rows.Count; j++)
                                {
                                    ...
                                    if (i != j)
                                    {
                                        ...
                                        {
                                            strRows += (" - " + jRows);
                                            exitsData = "Mã quận huyện '" + districtcodeCur + "' bị lặp trong file excel!" + strRows;
                                            resultEntry.AddMessage(exitsData);
                                            return Json(resultEntry);
                                        }
                                    }
                    ...
                    if (objRT_Mst_DistrictCur != null && objRT_Mst_DistrictCur.Lst_Mst_District != null)
                    {
                        ...
                        if (pageCount > 1)
                        {
                            for (var i = 1; i <= pageCount; i++)
                            {
                                ...
                                var objRT_Mst_DistrictExportCur = Mst_DistrictService.Instance.WA_RptSv_Mst_District_Get(objRQ_Mst_District, addressAPIs);
        ...
        private Dictionary<string, string> GetImportDicColumsTemplate()
        {
            ...
            {
                {TblMst_District.DistrictCode,"Mã quận huyện(*)"},
                {TblMst_District.ProvinceCode,"Mã tỉnh/thành phố (*)"},
                {TblMst_District.DistrictName,"Tên quận huyện (*)"}
            };
        }
        ...
        private string strWhereClause_Mst_District(Mst_District data)
        {
            ...
            {
                sbSql.AddWhereClause(Tbl_Mst_District + TblMst_District.DistrictCode, CUtils.StrValue(data.DistrictCode), "=");
            }
            if (!CUtils.IsNullOrEmpty(data.DistrictName))
            {
                sbSql.AddWhereClause(Tbl_Mst_District + TblMst_District.DistrictName, "%" + CUtils.StrValue(data.DistrictName) + "%", "like");
            }
