function Mst_InventoryLevelType() {
    this.ActionType = '';
    this.Image_Message = '';
    this.Confirm_Message = '';
    this.checkForm = function () {
        if (!commonUtils.checkElementIsNullOrEmpty('#InvLevelType', 'has-error-fix', 'Mã cấp kho không được trống!')) {
            return false;
        }
        if (!commonUtils.checkElementIsNullOrEmpty('#InvLevelTypeName', 'has-error-fix', 'Tên cấp kho không được trống!')) {
            return false;
        }
    ...
    this.getData = function () {
        ...
        var objMst_InventoryLevelType = new Object();
        objMst_InventoryLevelType.InvLevelType = commonUtils.returnValueText('#InvLevelType');
        objMst_InventoryLevelType.InvLevelTypeName = commonUtils.returnValueText('#InvLevelTypeName');
        ...
        if (commonUtils.checkElementExists('#FlagActive')) {
            ...
            objMst_InventoryLevelType.FlagActive = flagActive;
        }
        var modelCur = JSON.stringify(objMst_InventoryLevelType);
        var data = {
    ...
    this.deleteData = function (objMst_InventoryLevelType) {
        var invleveltype = commonUtils.returnValue(objMst_InventoryLevelType.InvLevelType);
        if (!commonUtils.isNullOrEmpty(invleveltype)) {
            var _ajaxSettings = this.ajaxSettings;
            bootbox.confirm({
