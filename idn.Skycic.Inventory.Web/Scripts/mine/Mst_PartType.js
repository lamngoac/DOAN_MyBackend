function Mst_PartType() {
    this.ActionType = '';
    this.Image_Message = '';
    this.Confirm_Message = '';
    this.checkForm = function () {
        if (!commonUtils.checkElementIsNullOrEmpty('#PartType', 'has-error-fix', 'Mã loại sản phẩm không được trống!')) {
            return false;
        }
        if (!commonUtils.checkElementIsNullOrEmpty('#PartTypeName', 'has-error-fix', 'Tên loại sản phẩm không được trống!')) {
            return false;
        }
    ...
    this.getData = function () {
        ...
        var objMst_PartType = new Object();
        objMst_PartType.parttype = commonUtils.returnValueText('#PartType');
        objMst_PartType.parttypename = commonUtils.returnValueText('#PartTypeName');
        if (!commonUtils.isNullOrEmpty(this.ActionType)) {
            if (this.ActionType === 'edit') {
                if (commonUtils.checkElementExists('#FlagActive')) {
                    ...
                    objMst_PartType.FlagActive = flagActive;
                }

            }
        }
        var modelCur = JSON.stringify(objMst_PartType);
        var data = {
    ...
    this.deleteData = function (objMst_PartType) {
        var parttype = commonUtils.returnValue(objMst_PartType.PartType);
        if (!commonUtils.isNullOrEmpty(parttype)) {
            var _ajaxSettings = this.ajaxSettings;
            bootbox.confirm({
