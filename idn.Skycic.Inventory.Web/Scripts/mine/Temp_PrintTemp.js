function Temp_PrintTemp() {
    ...
    this.checkForm = function() {
        if (!commonUtils.checkElementIsNullOrEmpty('#PrintTempCode', 'has-error-fix', 'Mã mẫu in không được trống!')) {
            return false;
        }
        return true;
    };
    ...
    this.getData = function() {
        ...
        var printTempDesc = commonUtils.returnValueText('#PrintTempDesc');
        ...
        var objTemp_PrintTemp = new Object();
        objTemp_PrintTemp.PrintTempCode = printTempCode;
        objTemp_PrintTemp.PrintTempDesc = printTempDesc;
        objTemp_PrintTemp.FilePath = filePath;

        var modelCur = commonUtils.returnJSONValue(objTemp_PrintTemp);
        var data = {
    ...
    this.deleteData = function (objTemp_PrintTemp) {
        var printtempcode = commonUtils.returnValue(objTemp_PrintTemp.PrintTempCode);
        if (!commonUtils.isNullOrEmpty(printtempcode)) {
            var _ajaxSettings = this.ajaxSettings;
            bootbox.confirm({
    ...
    this.approvedData = function (objTemp_PrintTemp) {
        var printtempcode = commonUtils.returnValue(objTemp_PrintTemp.PrintTempCode);
        if (!commonUtils.isNullOrEmpty(printtempcode)) {
            var _ajaxSettings = this.ajaxSettings;
            bootbox.confirm({
    ...
    this.cancelData = function (objTemp_PrintTemp) {
        var printtempcode = commonUtils.returnValue(objTemp_PrintTemp.PrintTempCode);
        if (!commonUtils.isNullOrEmpty(printtempcode)) {
            var _ajaxSettings = this.ajaxSettings;
            bootbox.confirm({
