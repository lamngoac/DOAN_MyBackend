		...
		var DocElement = function () {
			...
			(0, _createClass3.default)(DocElement, [{
				...
				value: function updateDisplayInternal(x, y, width, height) {
					if (this.el !== null) {
						var props = {
							left: this.rb.toPixel(x), top: this.rb.toPixel(y),
...
/***/ (function (module, exports, __webpack_require__) {
		...
		var Parameter = function () {
			...
			(0, _createClass3.default)(Parameter, [{
				...
				key: 'getFields',
				value: function getFields() {
					return ['id', 'name', 'type', 'arrayItemType', 'eval', 'nullable', 'pattern', 'expression', 'showOnlyNameType', 'testData', 'bgimage'];
				}
			}, {
				key: 'getId',
				value: function getId() {
...
/***/ (function (module, exports, __webpack_require__) {
		...
		var PopupWindow = function () {
			...
			(0, _createClass3.default)(PopupWindow, [{
				...
				value: function show(items, objId, tagId, field, type) {
					...
					if (type === PopupWindow.type.testData) {
						...
						this.elWindow.css({
							left: Math.round((winWidth - width) / 2) + 'px', top: Math.round((winHeight - height) / 2) + $(window).scrollTop() + 'px',
					...
					//tuyenba
					else if (type === PopupWindow.type.bgimage) {



						this.createBgImageSelector(items);
						var width = Math.round(winWidth * 0.8);
						var height = Math.round(winHeight * 0.8);
						this.elWindow.css({
							left: Math.round((winWidth - width) / 2) + 'px', top: Math.round((winHeight - height) / 2) + $(window).scrollTop() + 'px',
				...
				value: function hide() {
					if (this.visible) {
						...
						//tuyenba
						if (this.type === PopupWindow.type.bgimage) {

							var imgUrl = $('#rbro_document_pdf').attr('bgimage');
							var cmd = new _SetValueCmd2.default(this.rb.documentProperties.getId(), 'rbro_document_properties_bgimage', 'bgimage', imgUrl, _SetValueCmd2.default.type.text, this.rb);
							this.rb.executeCommand(cmd);
			...
			{
				//tuyenba
				key: 'createBgImageSelector',
				value: function createBgImageSelector(items) {

					var _this3 = this;

					var div = $('<div style="padding:20px;"><p>Select background image</p></div>');

					if (items.length > 0) {
						for (var i = 0; i < items.length; ++i) {
							var img = $('<img style="max-width:200px; height:283px; margin:10px;"/>');
							var a = $('<a href="javascript:;"/>').appendTo(div);
							img.attr('src', items[i]).appendTo(a);
							a.attr('imgurl', items[i]);

							a.click(function () {

								var imgurl = $(this).attr('imgurl');

								$('#rbro_document_pdf').css('background-image', 'url(' + imgurl + ')');
								$('#rbro_document_pdf').attr('bgimage', imgurl);
								_this3.hide();
							});
						}
					}
		...
		var DocumentProperties = function () {
			...
			(0, _createClass3.default)(DocumentProperties, [{
				...
				key: 'getFields',
					value: function getFields() {
					//tuyenba
						return ['pageFormat', 'pageWidth', 'pageHeight', 'unit', 'orientation', 'contentHeight', 'bgimage', 'marginLeft', 'marginTop', 'marginRight', 'marginBottom', 'header', 'headerSize', 'headerDisplay', 'footer', 'footerSize', 'footerDisplay', 'patternLocale', 'patternCurrencySymbol'];
						//--tuyenba
				}
			}, {
				key: 'getId',
		...
		var PageBreakElement = function (_DocElement) {
			...
			(0, _createClass3.default)(PageBreakElement, [{
				...
				value: function updateDisplayInternal(x, y, width, height) {
					if (this.el !== null) {
						var props = {
							left: this.rb.toPixel(0), top: this.rb.toPixel(y),
		...
		var BarCodeElement = function (_DocElement) {
			...
			(0, _createClass3.default)(BarCodeElement, [{
				...
				value: function updateDisplayInternal(x, y, width, height) {
					if (this.el !== null) {
						var props = {
							left: this.rb.toPixel(x), top: this.rb.toPixel(y),
							//tuyenba
							width: this.rb.toPixel(height), height: this.rb.toPixel(height)
							//--tuyenba
						};
						this.el.css(props);
					}
				...
				value: function createElement() {
					...
					this.elBarCode = $('<img src="data:image/png;base64," style="width: 100%; height: 100%;">');
					//--tuyenba
				...
				value: function createColumns(columns, isUpdate, insertColIndex, insertLeft) {
					...
					for (var _i = 0; _i < columns; _i++) {
						...
						if (useColDataIndex !== -1 && useColDataIndex < this.columnData.length) {
							...
							data = {
								band: this.band, columnIndex: _i, parentId: this.id, tableId: this.parentId,
		...
		var DocumentPropertiesPanel = function () {
			...
			(0, _createClass3.default)(DocumentPropertiesPanel, [{
				...
				value: function render(data) {
					...
					//tuyenba
					elDiv = $('<div class="rbroFormRow" id="rbro_document_properties_bgimage_row"></div>');
					elDiv.append('<label for="rbro_document_properties_bgimage">' + this.rb.getLabel('bgimage') + ':</label>');
					elFormField = $('<div class="rbroFormField"></div>');


					var elEditBgimageButton = $('<button id="rbro_document_properties_edit_bgimage"\n        class="rbroButton rbroActionButton" >\n                    <span>' + this.rb.getLabel('bgimage') + '</span>\n                    <span class="rbroIcon-edit"></span>\n                </button>').click(function (event) {


						//_this.rb.getPopupWindow().show(null, _this.documentProperties.getId(), 'rbro_document_properties_bgimage', 'bgimage', _PopupWindow2.default.type.bgimage);

						_this.rb.getPopupWindow().show(_this.rb.properties.bgimageList, _this.documentProperties.getId(), 'rbro_document_properties_bgimage', 'bgimage', 4);




					});
					elFormField.append(elEditBgimageButton);
					elFormField.append('<div id="rbro_document_properties_bgimage_error" class="rbroErrorMessage"></div>');
					elDiv.append(elFormField);
					panel.append(elDiv);
					//--tuyenba
				...
				value: function updateData(data) {
					if (data !== null) {
						...
						//tuyenba
						$('#rbro_document_properties_bgimage').val(data.getValue('contentHeight'));
						//--tuyenba
