...
namespace idN.TVAN.Web.Controllers
{
    public class Invoice_ImportExcelController : AdminBaseController
    {
        ...
        {
            System.Web.HttpContext.Current.Session["List_Invoice_Invoice_Sign"] = null;
            System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl_Sign"] = null;
            ...
            System.Web.HttpContext.Current.Session["FlagSignToken"] = null;
            System.Web.HttpContext.Current.Session["FileName"] = null;

            var userState = this.UserState;
            Session["UserState"] = userState;
            ViewBag.MST = userState.MstNNT.MST;
            ViewBag.NetworkID = userState.MstNNT.NetworkID;
            ...
            var flagSignToken = "1"; // 0: Ký mềm; 1: Ký bằng Usb Token
            if (!CUtils.IsNullOrEmpty(userState.MstNNT.CTSPath))
            {
                flagSignToken = "0";
            }
            //flagSignToken = "1";
            ViewBag.FlagSignToken = flagSignToken;
            System.Web.HttpContext.Current.Session["FlagSignToken"] = flagSignToken;
            ViewBag.ListInvoice_TempInvoice = listInvoice_TempInvoice;
            return View();
        }

        #region["Phát hành hóa đơn - Ký bằng Usb token"]

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CreateInvoice(string model)
        {
            ...
            var listInvoice_InvoiceDtl_Sign = new List<Invoice_InvoiceDtl>();
            ...
            var errorCode = "";
            var errorMessage = "";
            var invoice_Invoice_Update_Satus_List_Invoice_ImportExcel = new Invoice_Invoice()
            {
                InvoiceCode = "",
                InvoiceStatus = "",
            };
            try
            {
                if (System.Web.HttpContext.Current.Session["FileName"] != null)
                {
                    fileName = System.Web.HttpContext.Current.Session["FileName"] as string;
                }
                if (System.Web.HttpContext.Current.Session["List_Invoice_Invoice_Sign"] != null)
                {
                    listInvoice_Invoice_Sign = System.Web.HttpContext.Current.Session["List_Invoice_Invoice_Sign"] as List<Invoice_Invoice>;
                }
                if (System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl_Sign"] != null)
                {
                    listInvoice_InvoiceDtl_Sign = System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl_Sign"] as List<Invoice_InvoiceDtl>;
                }

                if (System.Web.HttpContext.Current.Session["List_Invoice_Invoice"] != null)
                {
                    listInvoice_Invoice = System.Web.HttpContext.Current.Session["List_Invoice_Invoice"] as List<Invoice_Invoice>;
                }
                if (System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl"] != null)
                {
                    //listInvoice_InvoiceDtl = System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl"] as List<Invoice_InvoiceDtl>;
                }
                ...
                //if (listInvoice_Invoice_Sign != null && listInvoice_Invoice_Sign.Count > 0 && listInvoice_Invoice != null &&
                //    listInvoice_Invoice.Count > 0 && listInvoice_InvoiceDtl != null && listInvoice_InvoiceDtl.Count > 0 &&
                //    listInvoice_ImportExcel != null && listInvoice_ImportExcel.Count > 0)
                if (listInvoice_Invoice_Sign != null && listInvoice_Invoice_Sign.Count > 0 && listInvoice_InvoiceDtl_Sign != null && listInvoice_InvoiceDtl_Sign.Count > 0 &&
                    listInvoice_ImportExcel != null && listInvoice_ImportExcel.Count > 0)
                {
                    #region["Xin số hóa đơn (Gọi hàm Biz thực hiện lưu và cấp số luôn)"]
                    var objInvoice_Invoice = Newtonsoft.Json.JsonConvert.DeserializeObject<Invoice_Invoice>(model);
                    invoice_Invoice_Update_Satus_List_Invoice_ImportExcel.InvoiceCode = objInvoice_Invoice.InvoiceCode;
                    var objInvoice_InvoiceInput = listInvoice_Invoice_Sign.FirstOrDefault(it => it.InvoiceCode.ToString().Trim().Equals(objInvoice_Invoice.InvoiceCode.ToString().Trim()));
                    var listInvoice_InvoiceDtl_SignCur = listInvoice_InvoiceDtl_Sign.Where(it => !CUtils.IsNullOrEmpty(it.InvoiceCode) && it.InvoiceCode.ToString().Trim().Equals(objInvoice_Invoice.InvoiceCode.ToString().Trim())).ToList();
                    var objRQ_Invoice_Invoice_SaveAndAllocatedInv = new RQ_Invoice_Invoice()
                    {
                        Tid = GetNextTId(),
                        ...
                        Lst_Invoice_Invoice = new List<Invoice_Invoice>() { objInvoice_InvoiceInput },
                        Lst_Invoice_InvoiceDtl = listInvoice_InvoiceDtl_SignCur,
                    };
                    
                    var objRT_Invoice_Invoice_SaveAndAllocatedInv = InvoiceInvoiceService.Instance.WA_Invoice_Invoice_SaveAndAllocatedInv(objRQ_Invoice_Invoice_SaveAndAllocatedInv, userState.AddressAPIs);
                    resultEntry.Success = true;
                    #endregion

                    #region["Update trạng thái hóa đơn vào listInvoice_ImportExcel"]
                    var objRT_Invoice_Invoice_Search = Invoice_Invoice_Get(objInvoice_Invoice.InvoiceCode.ToString().Trim());
                    if (objRT_Invoice_Invoice_Search.Lst_Invoice_Invoice != null &&
                        objRT_Invoice_Invoice_Search.Lst_Invoice_Invoice.Count > 0)
                    {
                        var objInvoiceUI_Update_InvoiceStatus = new Invoice_Invoice()
                        {
                        ...
                        var fileBase64Replace = CUtils.Base64_Encode(decode);
                        #endregion
                        System.Web.HttpContext.Current.Session["List_Invoice_ImportExcel"] = listInvoice_ImportExcel;
                        return Json(new { Success = resultEntry.Success, CertSerial = "", FileBase64 = fileBase64Replace, FileType = "0", InvoiceCode = objRT_Invoice_Invoice_Search.Lst_Invoice_Invoice[0].InvoiceCode.ToString().Trim() });
                    }
                    else
                    {
            ...
            catch (Exception ex)
            {
                ...
                IsResult = false;
                errorCode = resultEntry.ReturnErrorCode_ErrorMessage(ex, ref errorMessage);
                ...
                {
                    errorMessage = errorMessage + " ZZZZZ " + resultEntry.Detail;
                }
                ImportResult(ref listInvoice_ImportExcel, invoice_Invoice_Update_Satus_List_Invoice_ImportExcel, IsResult, errorCode, errorMessage);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ApprovedAndIssued(string model)
        {
            var resultEntry = new JsonResultEntry() { Success = false };
            var userState = this.UserState;
            Session["UserState"] = userState;
            ...
            try
            {
                    ...
                    listInvoice_ImportExcel != null && listInvoice_ImportExcel.Count > 0)
                {
                    ...
                    invoice_Invoice_Update_Satus_List_Invoice_ImportExcel.InvoiceCode = objInvoice_Invoice.InvoiceCode;
                    var objInvoice_InvoiceInput = listInvoice_Invoice_Sign.FirstOrDefault(it => it.InvoiceCode.ToString().Trim().Equals(objInvoice_Invoice.InvoiceCode.ToString().Trim()));
                    if (objInvoice_InvoiceInput != null)
                    {
                        objInvoice_InvoiceInput.InvoiceFileEncodeBase64 = objInvoice_Invoice.InvoiceFileEncodeBase64;

                        Invoice_Invoice_ApprovedAndIssued(new List<Invoice_Invoice>() { objInvoice_InvoiceInput });
                        IsResult = true;
                        objInvoice_InvoiceInput.InvoiceStatus = "ISSUED";
                    }
                    #endregion

                    #region["Gửi mail"]
                    var signOld = "xmlns=\"http://www.w3.org/2000/09/xmldsig#\"";
                    var contentsignOutputCur = CUtils.Base64_Decode(objInvoice_InvoiceInput.InvoiceFileEncodeBase64.ToString()).Replace(signOld, "");
                    
                    var serCUr = new SerializerXML();
                    var contentsignOutputCurObj = serCUr.Deserialize<Invoice>(contentsignOutputCur);
                    SendMailPost(contentsignOutputCurObj,ref objInvoice_InvoiceInput, userState, ref IsResult, ref errorCode, ref errorMessage);
                    #endregion
                    ImportResult(ref listInvoice_ImportExcel, objInvoice_InvoiceInput, IsResult, errorCode, errorMessage);
                    System.Web.HttpContext.Current.Session["List_Invoice_ImportExcel"] = listInvoice_ImportExcel;
                    resultEntry.Success = true;
                }
                else
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateImportResult(string model)
        {
            var resultEntry = new JsonResultEntry() { Success = false };
            var userState = this.UserState;
            Session["UserState"] = userState;
            ...
            try
            {
                ...
                if (System.Web.HttpContext.Current.Session["ListInvoice_InvoiceDtl"] != null)
                {
                    //listInvoice_InvoiceDtl = System.Web.HttpContext.Current.Session["ListInvoice_InvoiceDtl"] as List<Invoice_InvoiceDtl>;
                }
                ...
                if (listInvoice_Invoice_Sign != null && listInvoice_Invoice_Sign.Count > 0 && listInvoice_InvoiceDtl_Sign != null &&
                    listInvoice_InvoiceDtl_Sign.Count > 0 && listInvoice_ImportExcel != null && listInvoice_ImportExcel.Count > 0)
                {
                    var objInvoice_ImportExcel = Newtonsoft.Json.JsonConvert.DeserializeObject<Invoice_ImportExcel>(model);
                    var listInvoiceImportExcelCur = listInvoice_ImportExcel.Where(it => it.InvoiceCode.ToString().Trim().Equals(objInvoice_ImportExcel.InvoiceCode.ToString().Trim())).ToList();
                    if (listInvoiceImportExcelCur != null && listInvoiceImportExcelCur.Count > 0)
                    {
                        foreach (var item in listInvoiceImportExcelCur)
                        {
                            item.ImportResult = objInvoice_ImportExcel.ImportResult;
                        }
                    }
                }
        ...
        public ActionResult ImportResult()
        {
            ...
            return Json(new { Success = true, FileName = fileName, TongDongDuLieu = iTongDongDuLieu, TongSLHD = iTongSLHD, TongSLHDBoQua = iTongSLHDBoQua, TongSLHDThanhCong = iTongSLHDThanhCong, TongSLHDKoThanhCong = iTongSLHDKoThanhCong });
        }
        #endregion

        #region["Phát hành hóa đơn - Ký mềm"]

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult PhatHanhHoaDon()
        {
            var resultEntry = new JsonResultEntry() { Success = false };
            var userState = this.UserState;
            Session["UserState"] = userState;
            ...
            var listInvoice_Invoice_Sign = new List<Invoice_Invoice>();
            var listInvoice_InvoiceDtl_Sign = new List<Invoice_InvoiceDtl>();
            var listInvoice_SaveAndAllocatedInv_Sign = new List<Invoice_Invoice>();
            ...
            try
            {
                ...
                {
                    listInvoice_Invoice = System.Web.HttpContext.Current.Session["List_Invoice_Invoice"] as List<Invoice_Invoice>;
                    if (listInvoice_Invoice != null && listInvoice_Invoice.Count > 0)
                    {
                        iTongSLHD = listInvoice_Invoice.Count;
                    }
                }
                if (System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl"] != null)
                {
                    //listInvoice_InvoiceDtl = System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl"] as List<Invoice_InvoiceDtl>;
                    //if (listInvoice_InvoiceDtl != null && listInvoice_InvoiceDtl.Count > 0)
                    //{
                    //    iTongSLHD_ChiTiet = listInvoice_InvoiceDtl.Count;
                    //}
                }
                ...
                //if (iTongSLHD > 0 && iTongSLHD_ChiTiet > 0 && iTongDongDuLieu > 0)
                if (listInvoice_Invoice_Sign != null && listInvoice_Invoice_Sign.Count > 0 && listInvoice_InvoiceDtl_Sign != null && listInvoice_InvoiceDtl_Sign.Count > 0 && listInvoice_ImportExcel != null && listInvoice_ImportExcel.Count > 0)
                {

                    System.Web.HttpContext.Current.Session["List_Invoice_Invoice_Sign"] = listInvoice_Invoice_Sign;
                    System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl_Sign"] = listInvoice_InvoiceDtl_Sign;
                    System.Web.HttpContext.Current.Session["List_Invoice_Invoice"] = listInvoice_Invoice;
                    //System.Web.HttpContext.Current.Session["ListInvoice_InvoiceDtl"] = listInvoice_InvoiceDtl;
                    System.Web.HttpContext.Current.Session["List_Invoice_ImportExcel"] = listInvoice_ImportExcel;

                    #region["Lưu và cấp số hóa đơn - Ký - Phát hành hóa đơn - Gửi mail - Cập nhậtt hời gian gửi mail"]

                    if (listInvoice_Invoice_Sign != null && listInvoice_Invoice_Sign.Count > 0)
                    {
                        System.Web.HttpContext.Current.Session["List_Invoice_ImportExcel"] = listInvoice_ImportExcel;
                        var iCount = listInvoice_SaveAndAllocatedInv_Sign.Count;
                        SingleSign(new List<Invoice_Invoice>() { listInvoice_SaveAndAllocatedInv_Sign[0] }, listInvoice_InvoiceDtl_Sign);

                        var listInvoice_SaveAndAllocatedInv_SignSkip_1 = listInvoice_SaveAndAllocatedInv_Sign.Skip(1).Take(iCount - 1).ToList();
                        var pagesize = 100;
                        var pagecount = 0;
                        var totalItem = listInvoice_SaveAndAllocatedInv_SignSkip_1.Count;
                        pagecount = (totalItem % pagesize == 0) ? (totalItem / pagesize) : (totalItem / pagesize + 1);

                        for (int i = 0; i < pagecount; i++)
                        {
                            var listInvoice_Invoice_SignSkipCur = listInvoice_SaveAndAllocatedInv_SignSkip_1.Skip(pagesize * i).Take(pagesize).ToList();
                            MultiSign(listInvoice_Invoice_SignSkipCur, listInvoice_InvoiceDtl_Sign);
                        }
                ...
                {
                    exitsData = "Có lỗi xảy ra trong quá trình phát hành hóa đơn!";
        ...
        //        //if (iTongSLHD > 0 && iTongSLHD_ChiTiet > 0 && iTongDongDuLieu > 0)
        //        if (listInvoice_Invoice_Sign != null && listInvoice_Invoice_Sign.Count > 0 && listInvoice_InvoiceDtl != null && listInvoice_InvoiceDtl.Count > 0 && listInvoice_ImportExcel != null && listInvoice_ImportExcel.Count > 0)
        //        {
        //            #region["Xin cấp số tra cứu hóa đơn"]
        ...
        //            //    if (iTongSLHD == listSeq_InvoiceCode.Count)
        //            //    {
        //            //        var i = 0;
        //            //        foreach (var item in listInvoice_Invoice)
        //            //        {
        //            //            item.InvoiceCode = listSeq_InvoiceCode[i]; // Add số tra cứu hóa đơn vào item của listInvoiceUI
        //            //            var invoice_Idx = item.Invoice_Idx.ToString().Trim();

        //            //            #region["Add số tra cứu hóa đơn vào item listInvoiceDtlUI - listInvoice_ImportExcel"]
        //            //            var listInvoice_InvoiceDtlCur = listInvoice_InvoiceDtl.Where(it => !CUtils.IsNullOrEmpty(it.Invoice_Idx) && it.Invoice_Idx.ToString().Trim().Equals(invoice_Idx)).ToList();
        ...
        //            //        {
        //            //            // Tính toán thử hóa đơn thành công => Add item vào listInvoice_Invoice_Sign
        ...
        //            //            // Cập nhật trạng thái tính toán thử thành công vào listInvoiceImportExcel
        //            //            if (listInvoiceImportExcelCur != null && listInvoiceImportExcelCur.Count > 0)
        //            //            {
        ...
        //            //        else
        //            //        {
        //            //            // Tính toán thử hóa đơn không thành công
        //            //            // Cập nhật trạng thái tính toán thử không thành công vào listInvoiceImportExcel
        //            //            // Và không truyền cho Biz => trường hợp "Bỏ qua"
        //            //            if (listInvoiceImportExcelCur != null && listInvoiceImportExcelCur.Count > 0)
        //            //            {
        //            //                foreach (var it in listInvoiceImportExcelCur)
        //            //                {
        ...
        //            //                    else
        //            //                    {
        //            //                        if (errorMessage.Trim().Equals("Lỗi!"))
        //            //                        {
        //            //                            errorMessage = item.ErrorCode.ToString().Trim();
        //            //                        }
        //            //                    }

        //            //                    if (!CUtils.IsNullOrEmpty(item.ErrorDetail))
        //            //                    {
        //            //                        errorMessage = errorMessage + " ZZZZZ " + item.ErrorDetail.ToString().Trim();
        //            //                    }
        ...
        //                    GwPassword = GwPassword,
        //                };
        //                foreach (var item in listInvoice_Invoice_Sign)
        //                {
        //                    try
        //                    {
        //                        var listInvoice_InvoiceDtlCur = listInvoice_InvoiceDtl.Where(it => !CUtils.IsNullOrEmpty(it.InvoiceCode) && it.InvoiceCode.ToString().Trim().Equals(item.InvoiceCode)).ToList();

        //                        objRQ_Invoice_Invoice_SaveAndAllocatedInv.Tid = GetNextTId();
        //                        objRQ_Invoice_Invoice_SaveAndAllocatedInv.Lst_Invoice_Invoice = new List<Invoice_Invoice>() { item };
        //                        objRQ_Invoice_Invoice_SaveAndAllocatedInv.Lst_Invoice_InvoiceDtl = listInvoice_InvoiceDtlCur;

        //                        var objRT_Invoice_Invoice_SaveAndAllocatedInv = InvoiceInvoiceService.Instance.WA_Invoice_Invoice_SaveAndAllocatedInv(objRQ_Invoice_Invoice_SaveAndAllocatedInv, userState.AddressAPIs);

        //                        listInvoice_SaveAndAllocatedInv_Sign.Add(item);
        ...
        //                    catch (Exception ex)
        //                    {
        //                        // Thực hiện lưu và cấp số không thành công
        //                        // Update trạng thái hóa đơn vào listInvoice_ImportExcel
        //                        resultEntry.SetFailed().AddException(this, ex);
        //                        var IsResult = false;
        //                        var errorCode = "";
        //                        var errorMessage = "";
        ...
        //                            errorMessage = errorMessage + " ZZZZZ " + resultEntry.Detail;
        //                        }
        //                        ImportResult(ref listInvoice_ImportExcel, item, IsResult, errorCode, errorMessage);
        //                    }
        //                }
        //                System.Web.HttpContext.Current.Session["List_Invoice_ImportExcel"] = listInvoice_ImportExcel;
        //                #region["Update trạng thái hóa đơn vào listInvoice_ImportExcel và sinh nội dung file XML"]

        //                if (listInvoice_SaveAndAllocatedInv_Sign != null && listInvoice_SaveAndAllocatedInv_Sign.Count > 0)
        //                {
        //                    List<string> listInvoiceCode = listInvoice_SaveAndAllocatedInv_Sign.Select(it => it.InvoiceCode.ToString().Trim()).ToList();

        //                    var strInvoiceCode = string.Join(",", listInvoiceCode);
        //                    var objRT_Invoice_Invoice_Search = Invoice_Invoice_Get_Using_QueryIn_InvoiceCode(strInvoiceCode);

        //                    var listInvoice_Invoice_Search = new List<Invoice_Invoice>();
        //                    if (objRT_Invoice_Invoice_Search.Lst_Invoice_Invoice != null &&
        //                        objRT_Invoice_Invoice_Search.Lst_Invoice_Invoice.Count > 0)
        //                    {
        //                        listInvoice_Invoice_Search.AddRange(objRT_Invoice_Invoice_Search.Lst_Invoice_Invoice);
        //                    }

        //                    foreach (var item in listInvoice_SaveAndAllocatedInv_Sign)
        //                    {
        //                        var objInvoiceCur = listInvoice_Invoice_Search.FirstOrDefault(it => it.InvoiceCode.ToString().Trim().Equals(item.InvoiceCode.ToString().Trim()));
        //                        if (objInvoiceCur != null)
        //                        {
        //                            item.InvoiceCode = objInvoiceCur.InvoiceCode.ToString().Trim();
        //                            item.InvoiceNo = objInvoiceCur.InvoiceNo.ToString().Trim();
        //                            item.InvoiceStatus = objInvoiceCur.InvoiceStatus.ToString().Trim();

        //                            // Update trạng thái hóa đơn vào listInvoice_ImportExcel
        //                            var objInvoiceUI_Update_InvoiceStatus = new Invoice_Invoice()
        //                            {
        //                                InvoiceCode = objInvoiceCur.InvoiceCode.ToString().Trim(),
        //                                InvoiceNo = objInvoiceCur.InvoiceNo.ToString().Trim(),
        //                                InvoiceStatus = objInvoiceCur.InvoiceStatus.ToString().Trim(),
        //                                MailSentDateTime = null,
        //                            };
        //                            UpdateListInvoice_ImportExcel(ref listInvoice_ImportExcel, objInvoiceUI_Update_InvoiceStatus);
        //                            // Sinh nội dung file XML
        //                            var fileBase64 = GetXMLBase64(objInvoiceCur.InvoiceCode.ToString().Trim());
        //                            item.InvoiceFileEncodeBase64 = fileBase64;
        //                        }
        //                    }
        //                    System.Web.HttpContext.Current.Session["List_Invoice_ImportExcel"] = listInvoice_ImportExcel;
        //                    #region["Thực hiện ký - Phát hành - Gửi mail"]
        //                    var iCount = listInvoice_SaveAndAllocatedInv_Sign.Count;
        //                    SingleSign(new List<Invoice_Invoice>() { listInvoice_SaveAndAllocatedInv_Sign[0] }, listInvoice_InvoiceDtl);
        ...
        public ActionResult Import(HttpPostedFileWrapper file, string flagcheckcustomer, string flagconfirm, string invoicetype, string tinvoicecode, string formno, string sign)
        {
            ...
            System.Web.HttpContext.Current.Session["FileName"] = null;
            var flagSignToken = "1";
            if (System.Web.HttpContext.Current.Session["FlagSignToken"] != null)
            {
                flagSignToken = System.Web.HttpContext.Current.Session["FlagSignToken"] as string;
            }
            var resultEntry = new JsonResultEntry() { Success = false };
            var userState = this.UserState;
            Session["UserState"] = userState;
            ...
            try
            {
                ...
                System.Web.HttpContext.Current.Session["FileName"] = oldFileName;
                    ...
                    else
                    {
                        if (table.Rows.Count <= iMaxRows)
                        {
                            ...
                            foreach (var item in listInvoice_ImportExcel)
                            {
                                ...
                                var objInvoice_Invoice_Check = listInvoice_Invoice.FirstOrDefault(it =>
                                    (!CUtils.IsNullOrEmpty(it.Invoice_Idx) && it.Invoice_Idx.ToString().Trim().Equals(idx)));
                                if (objInvoice_Invoice_Check == null)
                                {
                                    var objInvoice_InvoiceCur = new Invoice_Invoice()
                                    {
                                        Invoice_Idx = idx,
                                        InvoiceCode = null,
                                    ...
                                    listInvoice_Invoice.Add(objInvoice_InvoiceCur);
                                }
                                ...
                                var objInvoice_InvoiceDtlCur = new Invoice_InvoiceDtl()
                                {
                                    InvoiceCode = null,
                                    Idx = idxDtl,
                                    Invoice_Idx = idx,
                                ...
                                listInvoice_InvoiceDtl.Add(objInvoice_InvoiceDtlCur);
                            ...
                            foreach (var item in listInvoice_Invoice)
                            {
                                ...
                                var listInvoiceDtl = listInvoice_InvoiceDtl.Where(it => (!CUtils.IsNullOrEmpty(it.Invoice_Idx) && it.Invoice_Idx.ToString().Trim().Equals(invoice_Idx))).ToList();
                                
                                if (listInvoiceDtl != null && listInvoiceDtl.Count > 0)
                                {
                            ...
                            #region["Sắp xếp theo ngày hóa đơn tăng dần"]
                            listInvoice_Invoice = listInvoice_Invoice.OrderBy(x => DateTime.Parse(x.InvoiceDateUTC.ToString())).ToList();
                            #endregion
                            ...
                            #region["Xin cấp số - Tính toán thử"]
                            #region["Xin cấp số tra cứu hóa đơn"]
                            var objRQ_Seq_InvoiceCode = new RQ_Seq_InvoiceCode()
                            {
                                Tid = GetNextTId(),
                                ...
                                objRT_Invoice_Invoice.Lst_Invoice_InvoiceCalc.Count > 0)
                            {
                                foreach (var item in objRT_Invoice_Invoice.Lst_Invoice_InvoiceCalc)
                                {
                                    ...
                                    if (item.ErrorCode.ToString().Trim().Equals("0"))
                                    {
                                        ...
                                        var listInvoice_InvoiceDtlCur = listInvoice_InvoiceDtl.Where(it => it.InvoiceCode.ToString().Trim().Equals(invoiceCodeCur)).ToList();

                                        if (listInvoice_InvoiceDtlCur != null && listInvoice_InvoiceDtlCur.Count > 0)
                                        {
                                            listInvoice_InvoiceDtl_Sign.AddRange(listInvoice_InvoiceDtlCur);
                                        }
                            ...
                            System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl_Sign"] = listInvoice_InvoiceDtl_Sign;
                            System.Web.HttpContext.Current.Session["List_Invoice_Invoice"] = listInvoice_Invoice;
                            System.Web.HttpContext.Current.Session["List_Invoice_InvoiceDtl"] = listInvoice_InvoiceDtl;
                            System.Web.HttpContext.Current.Session["List_Invoice_ImportExcel"] = listInvoice_ImportExcel;

                            if (flagSignToken.Equals("1"))
                            {
                                // Ký bằng  Usb Token
                                // thực hiện tính toán thử để giảm số hóa đơn bị lỗi trước khi trả về cho browser 
                                
                                var listInvoice_ImportExcel_Distinct = listInvoice_ImportExcel.GroupBy(it => it.Idx.ToString().Trim()).Select(g => g.First()).ToList();
                                var listInvoice_ImportExcel_Filter = new List<Invoice_ImportExcel>();
                                if (listInvoice_ImportExcel_Distinct != null &&
                                    listInvoice_ImportExcel_Distinct.Count > 0)
                                {
                                    listInvoice_ImportExcel_Filter = listInvoice_ImportExcel_Distinct.Where(it => (!CUtils.IsNullOrEmpty(it.FlagResult) && it.FlagResult.ToString().Trim().Equals("1"))).ToList();
                                }
                                return JsonView("LoadTableInvoice_ImportExcel", listInvoice_ImportExcel_Filter);
                            }
                            else
                            {
                                // Ký mềm
                                return Json(new { Success = true, TongSLHD = iTongSLHD });
                            }
        ...
        #region["Import Excel - comment"]
        //[HttpPost]
        //[ValidateAntiForgeryToken]
        //public ActionResult Import(HttpPostedFileWrapper file, string flagcheckcustomer, string flagconfirm, string invoicetype, string tinvoicecode, string formno, string sign)
        //{
        //    System.Web.HttpContext.Current.Session["List_Invoice_Invoice"] = null;
        //    System.Web.HttpContext.Current.Session["ListInvoice_InvoiceDtl"] = null;
        //    System.Web.HttpContext.Current.Session["List_Invoice_ImportExcel"] = null;
        //    System.Web.HttpContext.Current.Session["FileName"] = null;
        //    var resultEntry = new JsonResultEntry() { Success = false };
        //    var userState = this.UserState;
        ...
        //                    #region["Sắp xếp theo ngày hóa đơn tăng dần"]
        //                    listInvoiceUI = listInvoiceUI.OrderBy(x => DateTime.Parse(x.InvoiceDateUTC.ToString())).ToList();
        //                    #endregion

        //                    System.Web.HttpContext.Current.Session["List_Invoice_Invoice"] = listInvoiceUI;
        //                    System.Web.HttpContext.Current.Session["ListInvoice_InvoiceDtl"] = listInvoiceDtlUI;

        //                    try
        //                    {
        //                        var listDate = new List<string> { "2013-01-01", "2013-01-10", "2016-01-20" };
        //                        var orderedList = listDate.OrderByDescending(x => DateTime.Parse(x)).ToList();
        //                    }
        //                    catch
        //                    {
        ...
        //                    try
        //                    {
        //                        var orderedListUI = listInvoiceUI.OrderByDescending(x => DateTime.Parse(x.InvoiceDateUTC.ToString())).ToList();
        //                        var orderedListUI_ASC = listInvoiceUI.OrderBy(x => DateTime.Parse(x.InvoiceDateUTC.ToString())).ToList();
        //                    }
        //                    catch
        //                    {
        ...
        private void MultiSign(List<Invoice_Invoice> listInvoice_Invoice, List<Invoice_InvoiceDtl> listInvoice_InvoiceDtl)
        {
            ...
            var waUserCode = userState.SysUser.UserCode;
            var waUserPassword = userState.SysUser.UserPassword;
            var cTSPath = userState.MstNNT.CTSPath;
            var cTSPwd = userState.MstNNT.CTSPwd;
            ...
            for (int nScan = 0; nScan < nLoadSize; nScan++)
            {
                Thread t = new Thread((objParam) =>
                {
                    ...
                    try
                    {
                        ...
                        var decode = CUtils.Base64_Decode(nInvoice_InvoiceCur.InvoiceFileEncodeBase64.ToString()).Replace(@"&", "-");
                        ...
                        if (ESign.Sign("0", cTSPath, cTSPwd, ref SerialNumber, decode, ref contentsignOutput,
                                ref errorMessage, ref certdata) == true)
                        {
                            contentsignOutput = CUtils.Base64_Encode(contentsignOutput);
                            #region["Phat hanh"]
                            nInvoice_InvoiceCur.InvoiceFileEncodeBase64 = contentsignOutput;
                            Invoice_Invoice_ApprovedAndIssued(new List<Invoice_Invoice>() { nInvoice_InvoiceCur });
                            #endregion
                            nInvoice_InvoiceCur.InvoiceStatus = "ISSUED";
                            IsResult = true;

                            SendMailPost(contentsignOutputCurObj, ref nInvoice_InvoiceCur, userState, ref IsResult, ref errorCodeCur, ref errorMessageCur);
                        }
                        else
                        {
                            resultEntry.Success = false;
        ...
        private void MultiSign20190926(List<Invoice_Invoice> listInvoice_Invoice, List<Invoice_InvoiceDtl> listInvoice_InvoiceDtl)
        {
            var resultEntry = new JsonResultEntry() { Success = false };
            var userState = this.UserState;
            Session["UserState"] = userState;
            ...
            for (int nScan = 0; nScan < nLoadSize; nScan++)
            {
                Thread t = new Thread((objParam) =>
                {
                    ...
                    {
                        #region["Xin số hóa đơn (Gọi hàm Biz thực hiện lưu và cấp số luôn)"]
                        var listInvoice_InvoiceDtlCur = listInvoice_InvoiceDtl.Where(it => !CUtils.IsNullOrEmpty(it.InvoiceCode) && it.InvoiceCode.ToString().Trim().Equals(nInvoice_InvoiceCur.InvoiceCode)).ToList();
                        var objRQ_Invoice_Invoice_SaveAndAllocatedInv = new RQ_Invoice_Invoice()
                        {
                            Tid = GetNextTId(),
                            WAUserCode = waUserCode,
                        ...
                        #endregion
                        #region["Update trạng thái hóa đơn vào listInvoice_ImportExcel"]
                        var objRT_Invoice_Invoice_Search = Invoice_Invoice_Get(nInvoice_InvoiceCur.InvoiceCode.ToString().Trim());
                        if (objRT_Invoice_Invoice_Search.Lst_Invoice_Invoice != null &&
                            objRT_Invoice_Invoice_Search.Lst_Invoice_Invoice.Count > 0)
                        {
                            var objInvoiceUI_Update_InvoiceStatus = new Invoice_Invoice()
                        ...
                        if (ESign.Sign("0", cTSPath, cTSPwd, ref SerialNumber, decode, ref contentsignOutput, ref errorMessage, ref certdata) == true)
                        {
                            contentsignOutput = CUtils.Base64_Encode(contentsignOutput);
                            #region["Phát hành hóa đơn"]
                            nInvoice_InvoiceCur.InvoiceFileEncodeBase64 = contentsignOutput;
        ...
        private void SingleSign20190926(List<Invoice_Invoice> listInvoice_Invoice, List<Invoice_InvoiceDtl> listInvoice_InvoiceDtl)
        {
            var resultEntry = new JsonResultEntry() { Success = false };
            var userState = this.UserState;
            Session["UserState"] = userState;
        ...
        private RT_Invoice_Invoice Invoice_Invoice_Get_Using_QueryIn_InvoiceCode(string invoiceCode)
        {
            var listInvoice_Invoice = new List<Invoice_Invoice>();
            var userState = this.UserState;
            Session["UserState"] = userState;
            ...
            var waUserCode = userState.SysUser.UserCode;
            var waUserPassword = userState.SysUser.UserPassword;
            var strWhereClauseInvoice_Invoice = strWhereClause_Invoice_Invoice_QueryIn(new Invoice_Invoice() { InvoiceCode = invoiceCode });
            var objRQ_Invoice_Invoice = new RQ_Invoice_Invoice()
            {
                Tid = GetNextTId(),
                WAUserCode = waUserCode,
        ...
        private void SendMailPost(Invoice contentsignOutputCurObj, ref Invoice_Invoice invoiceInvoice, UserState userState, ref bool IsResult, ref string errorCodeCur, ref string errorMessageCur)
        {
            object moniter = new object();
            lock (moniter)
            {
                ...
                try
                {
                    ...
                    invoiceInvoice.MailSentDTimeUTC = strDateTime;
                    IsResult = true;
                    ...
                    if (invoice != null)
                    {
                        ...
                        if (listInvoiceImportExcelCur != null && listInvoiceImportExcelCur.Count > 0)
                        {
                            foreach (var it in listInvoiceImportExcelCur)
                            {
                                ...
                                it.MailSentDateTime = invoice.MailSentDTimeUTC;
                                if (isResult)
                                {
                                ...
                                else
                                {
                                    ...
                                    if (!CUtils.IsNullOrEmpty(invoice.InvoiceStatus) &&
                                        invoice.InvoiceStatus.ToString().Trim().Equals("ISSUED"))
                                    {
                                        it.ImportResult = "Lỗi gửi mail: " + errorMessage;
                                    }
                                    else
                                    {
        ...
        private void UpdateListInvoice_ImportExcel(ref List<Invoice_ImportExcel> listInvoice_ImportExcel, Invoice_Invoice invoice)
        {
            object moniter = new object();
            lock (moniter)
            {
        ...
        private string strWhereClause_Invoice_Invoice_QueryIn(Invoice_Invoice data)
        {
            var strWhereClause = "";
            var sbSql = new StringBuilder();
            var Tbl_Invoice_Invoice = TableName.Invoice_Invoice + ".";
            ...
            if (!CUtils.IsNullOrEmpty(data.InvoiceCode))
            {
                sbSql.AddWhereClause(Tbl_Invoice_Invoice + TblInvoice_Invoice.InvoiceCode, CUtils.StrValue(data.InvoiceCode), "in");
            }
